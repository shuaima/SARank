...
We discuss the necessity and the selection of parameters right after Eq. (7) (left column, page 4). More specifically, these parameters make our model be able to fit to the various ranking scenarios. As will be seen in the experiments, our model performs well in two reasonable ranking scenarios by using quite different aggregating parameters, and, moreover, these parameters are indeed quite flexible to choose within a certain range.
%
Note that a) the optimal \PairAcc is obtained within a single region, b) the \PairAcc keeps at a high level within a certain ($\alpha$, $\beta$) combination space around the optimal region, and c) the optimal parameters on the same set of ground-truth are very similar across different datasets (Exp-5.3, page 10).
...
%
This work focuses on {\em query independent ranking} because of its wide application in modern scholarly search engines such as Google Scholar, Semantic Scholar and Microsoft Academic Search.
%
And it can be easily extended to {\em query dependent ranking}. Our ranking can serve as an indicator of the importance of items, and, given a specific query, it can either be directly used to rank a subset of relevant items or combine with any relevance scores for ranking.
%
...
An alternative way to do that is to directly change node values. However, it is difficult to handle as importance does not have a clear and closed-form definition and it may also cause problems for convergence.
To our knowledge, there is no related work on changing node weights to achieve this goal and it is yet unclear how to handle it. Finally, we use iterations with impact weights since importance is essentially propagable through edges and it is also fair for all nodes.
...
Figure 1 shows the logscale percentage of the number of articles/citations of each year which grows exponentially with time, instead of the number of citations (\ie impacts) of individual articles.
...
Moreover, the aging function in [17] exploits a log-normal survival probability which also only decays with time after the citation peak.
...
Yes, we have revised our paper as suggested (right column, page 1 \& Table I, page 5).
...
We have reported the memory costs in a new set of experiments, \ie Exp-4 (page 10). 
On the large \aminer and \magdata, algorithm \pagerank uses the least memory, followed by algorithms \futurerank, \ensemblerank and \hhgrank, respectively.
%
The memory used by \pagerank is even less than the size of the academic graph data as it is based on the citation graph only.
%
Our \ensemblerank model costs more memory than \futurerank since it also combines article-venue relationships ignored by \futurerank and it assembles multiple rankings based on prestige and popularity. However, our \ensemblerank model is still more memory-efficient than \hhgrank.
%
To conclude, the \ensemblerank model uses reasonable amount of  memory to achieve the better effectiveness.
...
Yes, we add the volume statistics (the number of articles published at each year) of the growing data  in Figure 1.
...
Yes. We have discussed the correlation between our work and temporal graphs and included necessary references in Related Work.
...
\textbf{[D1]} \emph{The reviewers see some merit in the approach, but they have also raised important concerns regarding the technical depth and novelty of the approach. In particular, the authors use known techniques for an application (scientific article ranking) for which there is abundance of previous work (it even has its own field, called scientometrics, or  bibliometrics).}


The techniques used in our work are not simply existing techniques:
a) an importance assembling based ranking model is still novel in the field of scientometrics and bibliometrics and we further consider the efficient batch and dynamic computation problem which rarely has been exploited in these two fields, b) an incremental algorithm for block-wise time-weighted PageRank computation that makes use of the particular properties of the scholarly graphs, and c) a thorough experimental study.
That is, although bearing similarities, this paper certainly has its contributions to the technical part and brings certain novelty for the task.

Please also refer to our response to R2W1\&D1 for more details.
