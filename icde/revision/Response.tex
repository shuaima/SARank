...
And our ranking can serve as an indicator of the importance of items, and, given a specific query, it can either be directly used to rank a subset of relevant items or combine with any relevance scores for ranking.
%
The extension in other heterogeneous information networks is currently unclear, since the TWPageRank is derived from an analysis of impacts of articles (left column, page 2). We shall identify possible applications in the future.
...
Figure 1 shows the logscale percentage of the number of articles/citations of each year which grows exponentially with time, instead of numbers of citations (\ie impacts) of individual articles.
We have highlighted it in the revised version (Figure 1, page 2).

We have explained our claim better. The observation is inspired by the five distinct citation patterns. Considering that the direct decay model only conforms with {\em MonDec}, we thus propose our claim that can better explain the five citation patterns. Moreover, reference [17] exploits a log-normal survival probability which also decays with time after the citation peak.
...
Yes, we have revised our paper as suggested (right column, page 1 \& right column, page 5).
...
We think that the techniques proposed also have their own interests: a) An importance assembling based ranking model itself is hard to define and tune, b) an incremental algorithm that makes use of the particular properties of the scholarly graphs, and c) a thorough experimental study. That is, this paper certainly has its contributions to the technical part.
...
%
Please also refer to our response to R2W1.
...
We have removed the sentence and explained more details about the statement. Moreover specifically, we add the details of the generation of the author citation graph. The generated graph has 7.12 billion edges on the largest \magdata (\ie the memory cost of storage only is 57GB), which makes it computationally expensive to compute prestige of authors  with TWPageRank on that graph (Author Component, left column, page 3). We hence turn to the alternative light-weight method to achieve the goal.
...
We have reported the memory cost in Exp-3 (page 10) that on the largest \magdata, (\futurerank, \ensemblerank) cost (16.42GB, 49.42GB) memory when $Y_s=2015$, and \hhgrank costs 61.37GB memory when $Y_s=2014$. Interestingly, our \ensemblerank model costs more memory than \futurerank due to its model complexity, while it is more efficient with our batch and incremental algorithms.
