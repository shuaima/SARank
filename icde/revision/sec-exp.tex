...
\label{tab-result}
\begin{center}
\caption{\small Memory cost of algorithms}
%\begin{small}
\vspace{-.5ex}
\begin{tabular}{|c|c|c|c|c|c|}
\hline
{\bf Datasets}   &  \hspace{1ex}Original\hspace{1ex}     &  \hspace{1ex}\pagerank\hspace{1ex}     & \hspace{1ex}\futurerank\hspace{1ex}  &  \hspace{1ex}\hhgrank\hspace{1ex}  &   \hspace{1ex}\ensemblerank\hspace{1ex}    \\
\hline \hline
...
In the fourth set of tests, we evaluated the memory cost of our algorithm. For a fair comparison, all algorithms used all data on both \aan and \aminer, and used data before 2014 on \magdata, since the \hhgrank will run out of memory in when $Y_s=2015$. The original data including paper citation graph, paper-author graph, paper-venue graph and time information of articles. The results of memory cost are reported in Table~IV.
