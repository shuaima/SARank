...
In the fourth set of tests, we evaluated the memory cost of our algorithm. For a fair comparison, all algorithms used data before 2014, since the \hhgrank will run out of memory in 2015. The results of memory cost are reported in Table~IV.
The original data including
\markedb{The memory cost is an important factor of efficiency. We thus collect the memory use on the largest \magdata that (\futurerank, \ensemblerank) cost (16.42GB, 49.42GB) memory when $Y_s=2015$, and \hhgrank costs 61.37GB when $Y_s=2014$. Interestingly, our \ensemblerank model costs more memory than \futurerank due to its model complexity, while it is more efficient with our batch and incremental algorithms.}
