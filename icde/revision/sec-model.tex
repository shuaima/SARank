...
\vspace{-1ex}
\caption{\small Citation statistics of scholarly articles that reports the {\em logscale percentage of the number of citations of each year} \wrt\ the total number of citations on \aan, \aminer and \magdata, respectively.}
\label{fig-reference}
\vspace{-3ex}
\end{figure}
...
\includegraphics[scale=0.175]{fig/framework-lite-2.eps}
\vspace{-1ex}
...
\markedb{To evaluate the importance of each author, an author citation graph needs firstly to be constructed such that (a) a node represents an author, and (b) a direct edge $(s,t)$ denotes that there exist articles published by author $s$ citing articles published by author $t$. It is not hard to image that the number of edges in the author citation graph is much larger than the number in the citation graph, which reaches xxx on \magdata. Handing such a graph is computationally expensive. }
...
The final ranking score $R(v)$ of an article $v$ is aggregated as follows:
...
%
\markedb{
By this, our model is able to fit to the various ranking scenarios in real-life applications, which hardly can be covered with one set of parameters. As will be shown in the experiments, our model performs well in two reasonable ranking scenarios by selecting quite different aggregating parameters, and, moreover, these parameters are indeed quite flexible to choose within a certain range, \ie $[0,1]$. }
