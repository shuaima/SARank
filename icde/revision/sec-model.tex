...
\markedb{To do this, an {\em author graph} needs to be constructed such that (a) a node represents an author, and (b) a direct edge $(s,t)$ denotes that there exist articles of author $s$ citing articles of author $t$. It is not hard to image that the number of edges in the author graph is much larger than the number in the citation graph, which reaches to 7.12  billion on the largest \magdata (\ie the memory cost of storage only is 57GB). It is computationally expensive to compute prestige of authors with TWPageRank on that graph.}
