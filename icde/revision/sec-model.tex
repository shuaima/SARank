...
\includegraphics[scale=0.41]{fig/yearly-papers.eps}}
\hspace{-4ex}
\subfigure{\label{fig-citation}
\includegraphics[scale=0.41]{fig/yearly-references.eps}}
\caption{\small Statistics of scholarly articles that reports the {\em logscale percentage of the number of papers (left) and citations (right) of each year} \wrt\ the total number of papers and citations on \aan, \aminer and \magdata, respectively.}
...
\markedb{To do this, an {\em author citation graph} needs to be constructed such that (a) a node represents an author, and (b) a direct edge $(s,t)$ denotes that there exist articles of author $s$ citing articles of author $t$. It is not hard to image that the number of edges in the author citation graph is much larger than the number in the citation graph, which reaches to 7.12  billion on the largest \magdata (\ie the memory cost of storage only is 57GB). It is computationally expensive to compute prestige of authors with TWPageRank on that graph.}
