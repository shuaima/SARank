...
%
To unify these distinct citation patterns, we adopt that {\em the impact of an article tends to decay with time after the peak time}, as pointed out in~\cite{WangSB13}. That is, the impacts of articles directly decay with time only for those in {\em MonDec}, and decay with time after the (highest) peak time for those in {\em PeakInit}, {\em PeakLate} and {\em PeakMul}, and do not decay for ones in {\em MonIncr} (which is rare).
