...
%
To our knowledge, the Time-Weighted PageRank is among the first to \markedb{use citation statistics to incorporate the diverse citation patterns of individual articles for scholarly article ranking.}
%and the long-term bias caused by the dynamic nature of data

\sstab(2)  We then develop  a batch algorithm for scholarly article ranking (Section \ref{sec-alg}), in which we propose a block-wise method for Time-Weighted PageRank \markedb{and show its advantage in terms of an efficiency analysis based on citation characteristics of scholarly articles.}
